...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    //float alpha = pressed ? 1.0f : 0.0f;

    //float ANIM_SPEED = 0.25f;
    //if (g.LastActiveId == g.CurrentWindow->GetID(label)) {
    //    float t_anim = ImSaturate(g.LastActiveIdTimer / ANIM_SPEED);
    //    alpha = pressed ? (t_anim) : (1.0f - t_anim);
    //}
    // Render
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    float alpha = *v ? 1.0f : 0.0f;

    float ANIM_SPEED = 0.25f;
    if (g.LastActiveId == g.CurrentWindow->GetID(label)) {
        float t_anim = ImSaturate(g.LastActiveIdTimer / ANIM_SPEED);
        alpha = *v ? (t_anim) : (1.0f - t_anim);
    }
...
bool ImGui::SubTabEx(const char* label, const char* icon, const bool selected, const ImVec2& size_arg)
{
    ...
    window->DrawList->AddText(ImVec2(bb.Min.x + size_arg.x / 2 - ImGui::CalcTextSize(label).x / 2, bb.Min.y + size_arg.y / 2 - ImGui::CalcTextSize(label).y / 2), selected ? ImColor(255, 255, 255, 255) : ImColor(255, 255, 255, 175), label);
