...
private:
	...
	constexpr T call_vfunc(void* thisptr, std::size_t nIndex, args_t... argList)
	{
		using virtual_fn = T(__thiscall*)(void*, decltype(argList)...);
		return (*static_cast<virtual_fn**>(thisptr))[nIndex](thisptr, argList...);
	}
public:
	void draw_screen_space_rectangle(i_material* pMaterial, int iDestX, int iDestY, int iWidth, int iHeight, float flTextureX0, float flTextureY0, float flTextureX1, float flTextureY1, int iTextureWidth, int iTextureHeight, void* pClientRenderable = nullptr, int nXDice = 1, int nYDice = 1)
	{
		call_vfunc<void>(this, 114, pMaterial, iDestX, iDestY, iWidth, iHeight, flTextureX0, flTextureY0, flTextureX1, flTextureY1, iTextureWidth, iTextureHeight, pClientRenderable, nXDice, nYDice);
	}
};
...
public:
	...
	i_material_render_context* get_render_context() {
		return call_vfunc<i_material_render_context*>(this, 115);
		//using fn = i_material_render_context*(__thiscall*)(void*);
